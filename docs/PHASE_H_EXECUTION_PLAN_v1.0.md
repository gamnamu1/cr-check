...
예시:
  1-1 (사실 검증 부실) ← 상위 유지 (hierarchy_level=2)
    1-1-1-a: 교차 검증 부재 및 단일 취재원 의존
    1-1-1-b: 취재원 발언의 무비판적 중계
    1-1-1-c: 인용 정보의 출처 및 신뢰성 문제
    1-1-1-d: 반론권 미보장
```
...
각 소분류(###) 아래 불릿 항목을 1-1-1-a, 1-1-1-b 형식으로 코드화한다.
```
...
신규 코드상위 코드개념명1-1-1-a1-1교차 검증 부재 및 단일 취재원 의존...
```
...
예: 기존 1-1 → 해당 하위 코드(1-1-1-a, 1-1-1-b 등)로 구체화.
...
확인 포인트:
  ...
  (C) 코드 명명 — 1-1-1-a 형식이 일관되게 적용됐는가
