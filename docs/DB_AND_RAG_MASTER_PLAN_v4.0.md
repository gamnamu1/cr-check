...
| 패턴 레코드 수 | ~102개 (소분류 단위) | 38개 (소분류 압축 + 대분류 8개 포함) | M2 단계 의식적 압축 결정 |
| 현재 매핑 건수 | 설계 목표 300~500건 | 63건 (Phase γ 정비 후) | 4층 편중 계통 문제 (Phase G에서 복원 중) |
...
3. **38개 압축 패턴의 임베딩 희석**: 여러 독립 개념이 하나의 description에 압축되어 벡터 방향성 분산. Phase H(38→119 세분화) 미결정.
