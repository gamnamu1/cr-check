...
> **최종 수정**: 2026-04-21 (현행 운영 현황 동기화: Sonnet Solo 구조·threshold·인용방식·TN 기준·체크리스트 반영)
> **운영 방식**: 전 Phase 익명 운영 (Auth 없음), 분석 결과는 공개 URL로 접근·공유

---

## 0. 현행 운영 현황 (프로덕션 기준 — 2026-04-21)

> **목적**: 이 섹션은 §4·§7의 설계 원본과 실제 프로덕션 구현 간의 괴리를 명시한다.
> 새 세션을 시작하는 CLI·감리자는 설계 원본보다 **이 섹션을 먼저 읽고** 현재 상태를 기준으로 삼아야 한다.

### 0.1 현재 파이프라인 아키텍처 (실제)

```
기사 URL → 스크래핑 → 전처리·청킹 → 임베딩
    ↓
search_pattern_candidates() — threshold=0.2, Top-7
    ↓
벡터 후보 패턴에 ★ 표시 후, 38개 패턴 전체 목록을 Sonnet 4.5에 제공
(벡터 후보는 "힌트"로만 활용, 완전한 사전 필터가 아님)
    ↓
Claude Sonnet 4.5 — Devil's Advocate CoT (Phase 1)
★ 표시 패턴 우선 검토 + 38개 전체도 함께 제공
구조적 패턴(1-7-2 등)은 체크리스트로 별도 판단
    ↓
확정된 패턴 ID → get_ethics_for_patterns() → 규범 조회
    ↓
Claude Sonnet 4.6 — 3종 리포트 생성 (시민·기자·학생) (Phase 2)
인용 방식: 〔규범 원문〕 마커 직접 출력
    ↓
최종 리포트 저장·공유
```

### 0.2 설계 원본 대비 변경된 핵심 결정

| 항목 | 설계 원본 (§4·§7) | 현행 운영 (실제) | 변경 이유 |
|---|---|---|---|
| RAG 호출 구조 | Haiku(패턴 확정) + Sonnet(리포트), 1.5회 | Sonnet 4.5 단독 패턴 확정 + CoT, ★마커 힌트 구조 | 품질 우선. Haiku 단독 패턴 판단의 정밀도 한계 |
| 벡터 검색 파라미터 | threshold=0.5, Top-10 | threshold=0.2, Top-7 | 낮은 threshold로 CR 확보 우선 |
| 패턴 제공 방식 | 벡터 후보만 Haiku에 전달 (진정한 RAG 필터) | 38개 전체 + ★힌트로 Sonnet에 전달 | ★마커 시스템 도입으로 실질적 전수 검토 |
| 인용 방식 | `<cite ref="..."/>` 태그 + 백엔드 DB 치환 | 〔규범 원문〕 마커 직접 출력 | CitationResolver 비활성화(Phase β 이후) |
| TN 안전 기준 | TN 100% 절대 유지 | TN FP Rate 50% 이하 허용 (6건 중 3건 이하) | 실용적 기준으로 완화 (2026-04-20 확정) |
| 패턴 레코드 수 | ~102개 (소분류 단위) | **149건** (대분류 8 + 부모 34 + leaf 107) — Phase H STEP 2 완료 (2026-05-03) | Phase H 세분화 완료 |
| 현재 매핑 건수 | 설계 목표 300~500건 | **활성 leaf 76개 전원 매핑 (총 111건+)** — Phase H STEP 3 완료 (2026-05-03), PCP 44.6% | Phase H STEP 3 완료 |

### 0.3 체크리스트 패턴 (구조적 판단 필수)

벡터 검색으로 탐지가 어려운 구조적 패턴은 Sonnet 4.5에게 체크리스트 형태로 별도 판단을 요청한다.

| 패턴 코드 | 패턴명 | 체크리스트 적용 이유 |
|---|---|---|
| 1-7-2 | 헤드라인 윤리 | 제목-본문 간 의미 비교 필요, 어휘 매칭 불가 |
| 1-3-1 | 관점 다양성 부족 | 취재원 수·다양성 계산, 전체 구조 파악 필요 |
| 1-6-1 | 심층성 부족 | 기사 전체 깊이 판단, 어휘 매칭 불가 |
| 1-3-4 | 갈등 조장 | 전체 논조·프레이밍 분석 필요 |

### 0.4 현재 벤치마크 기준 (R4, 2026-04-20)

| 지표 | 수치 | 비고 |
|---|---|---|
| Candidate Recall (CR) | 49.2% | 벡터 검색 단계 성능 |
| Final Recall (FR) | 26.7% | 최종 탐지율 |
| Precision | 30.4% | 확정 패턴의 정확도 |
| TN FP Rate | 2/6 | 허용 기준 50% 이하 ✅ |

> **핵심 관찰**: CR>0인 케이스에서도 Sonnet 최종 확정률 ≈54%. 벡터가 정답을 찾아줘도 Sonnet 판단 단계에서 ~46%가 탈락. description 재작성(T-트랙)만으로는 이 병목을 해소할 수 없음.

### 0.5 현재 알려진 병목 및 미해결 과제

1. **(가)/(나) 이진 게이트**: Devil's Advocate CoT에서 "양질 근거(가)"가 우세하면 detections=[] 처리. 양질이면서 특정 측면에서 문제가 있는 기사를 전부 기각. **다음 실험 대상 (PERFORMANCE_IMPROVEMENT_PLAN 참조).**
2. **description 단일 필드 이중 목적 충돌**: 벡터 검색용(구체 어휘)과 리포트 생성용(품격 있는 정의)이 하나의 필드를 공유. T-트랙 어휘 추가가 리포트 tone과 트레이드오프. `search_text` 필드 분리 검토 중.
3. **38개 압축 패턴의 임베딩 희석**: **(해소)** Phase H STEP 2~5 완료 (107개 leaf 세분화, search_text 분리, 프롬프트 재설계). STEP 6(임베딩 재생성) 대기 중.
4. **리포트 품질 기준선 부재**: 탐지 성능(FR/Precision) 외에 최종 리포트가 실제로 나아졌는지를 측정하는 지표 없음.
...
> ⚠️ **현행 운영값과 상이함** — 실제 운영 파라미터는 §0.2 참조.
...
[현행 운영 — 2026-04-21 기준]
match_threshold = 0.2  (더 느슨하게, CR 확보 우선)
match_count = 7        (상위 7개로 제한)
```

- 느슨한 threshold로 관련 패턴의 누락(recall 저하)을 방지
- Top-K로 무관한 패턴의 유입(precision 저하)을 제한
- ~~Haiku가 컨텍스트를 읽고 무관한 결과를 걸러내는 "LLM as filter" 역할~~ → 현재는 Sonnet 4.5가 ★마커 힌트와 함께 전체 38개 패턴을 검토
- threshold 값은 골든셋 기반으로 경험적 튜닝

### 결정 E — RAG 호출 구조 ⭐

> ⚠️ **현행 운영 구조와 상이함** — 실제 구조는 §0.1·§0.2 참조.
...
**[현행 운영] Sonnet Solo + ★마커 힌트 구조**:

```
기사 → 청킹 → 임베딩 → 벡터검색(threshold=0.2, Top-7)
    → ★마커 표시 후 38개 패턴 전체를 Sonnet 4.5에 제공
    → Sonnet 4.5 (Devil's Advocate CoT + 체크리스트)
    → 확정된 패턴 ID로 규범 정밀 조회 (DB JOIN)
    → Sonnet 4.6 (3종 리포트 생성)
```

**설계 원본의 핵심 가치 (변경되지 않은 부분)**:
- Sonnet에게는 확정 패턴에 연결된 규범만 정밀 전달 → 사각지대 제거
- "컨텍스트에 없어서 규범을 인용 못하는" 문제(Silent Miss)를 구조적으로 해소

**변경된 부분 및 이유**:
- Haiku 단계 제거: 패턴 판단 품질을 위해 Sonnet 직접 판단으로 전환
- 전체 패턴 제공: ★마커로 벡터 힌트를 주되, 38개 전수 검토로 누락 방지
- 단, 이 구조에서 이진 게이트 문제(§0.5) 발생 — 개선 실험 예정

### 결정 F — 인용 아키텍처 ⭐

> ⚠️ **현행 운영 방식과 상이함** — 실제 인용 방식은 §0.2 참조.

**[설계 원본] 결정론적 인용(Deterministic Quoting)**
...
CitationResolver(`citation_resolver.py`)가 Phase β에서 비활성화됨에 따라,
Sonnet 4.6이 규범 원문을 `〔규범 원문〕` 마커 형태로 직접 출력한다.
- 환각 위험은 프롬프트 레벨에서 DB 조회 원문을 컨텍스트로 직접 제공하여 억제
- 완전한 결정론적 치환은 아니나, 컨텍스트 내 원문을 활용하므로 실용적 정확도 유지

**가치 (변경되지 않은 부분)**: 규범 원문을 컨텍스트로 직접 제공하여 환각을 억제하는 원칙은 유지.
...
> **STEP 0-B 연동 (2026-05-01 추가):** 이 롤업 구조가 실제로 작동하려면
> `ethics_codes.applicable_contexts` 레이블링이 완료되어야 한다.
> 기사 유형별 필터링(applicable_contexts)과 Tier 역순 정렬(구체→포괄)이
> 조합될 때 "가장 날카로운 조항이 가장 먼저 인용"되는 효과가 완성된다.
> 상세 설계: `STEP0B_ETHICS_PLAN_v1.0.md` §7 참조.
...
- **Pattern Precision**: Haiku(현재는 Sonnet 4.5)가 확정한 패턴 중 정답과 일치하는 비율
- **Citation Accuracy**: 리포트의 규범 인용이 정답 규범과 일치하는 비율
- **Faithfulness**: 생성된 리포트가 컨텍스트에 충실한 정도

**[현행 운영 — 실제 측정 지표]** (R1~R4 벤치마크 기준)
- **CR (Candidate Recall)**: 벡터 검색 후보 풀에 정답 패턴 포함 여부
- **FR (Final Recall)**: Sonnet 최종 확정 패턴 중 정답 패턴 포함 비율
- **Precision**: 확정 패턴 중 정답과 일치하는 비율
- **TN FP Rate**: TN 케이스(정상 기사)에서 오탐 발생 비율

> **미측정 항목**: Citation Accuracy, Faithfulness, 리포트 품질 정성 평가.
> 이 지표들이 없어서 DB 매핑 개선이나 description 재작성이 실제 리포트 품질을 높였는지 확인 불가.
> **리포트 품질 기준선 구축이 필요** (골든셋 10건 대상 사람 평가 점수 사전 설정).
...
> ⚠️ **이 테이블은 설계 원본 기준**. 현행 운영 상태는 §0.2의 변경 이력 테이블을 우선 참조.

| 항목 | 설계 원본 | 현행 운영 (변경된 경우) |
|------|------|------|
| RAG 방식 | Hybrid RAG (pgvector + 관계형 테이블) | 동일 |
| 인프라 | Supabase 단일 플랫폼 | 동일 |
| 임베딩 모델 | OpenAI text-embedding-3-small 1차 | 동일 |
| 청킹 (패턴/규범) | 자연 구조 그대로 | 동일 |
| 청킹 (기사) | 의미 기반 병합 청킹 (300~500자) | 동일 |
| 검색 전략 | Top-K(10) + threshold(0.5) | **Top-K(7) + threshold(0.2)** |
| 관계 모델링 | 관계형 테이블 | 동일 |
| 규범 인용 방식 | 결정론적 인용 (cite 태그 + DB 치환) | **〔〕마커 직접 출력** |
| RAG 호출 구조 | 1.5회: 벡터검색→Haiku→규범 조회→Sonnet | **Sonnet Solo + ★마커 힌트 구조** |
| 메타 패턴 추론 | 하이브리드 (규칙 사전필터 + LLM + 가드레일) | 동일 |
| TN 안전 기준 | TN 100% 절대 유지 | **TN FP Rate 50% 이하 허용** |
| 규범 버전 관리 | Immutable Records + 스냅샷 | 동일 |
| Eval 체계 | Phase 0 병행, 골든셋 20~30건, Recall@K 기반 | 골든셋 26건 운영 중. Citation Accuracy·Faithfulness 미측정 |
| Phase 실행 순서 | Phase 0 + 1 동시 진행 | D·E·F·G 완료. 현재 성능 개선 트랙 진행 중 |
| 사용자 인증 | 없음 — 전 Phase 익명 운영 | 동일 |
| 기존 스택 | FastAPI(BE) + Next.js(FE) + Railway/Vercel | 동일 |
...
| Vector Search | pgvector | RAG 임베딩 검색 |
| Backend | FastAPI + supabase-py | API + DB 연동 |
| Frontend | Next.js 15 | UI |
| AI Phase 1 (패턴 확정) | Claude Sonnet 4.5 (`claude-sonnet-4-5-20250929`) | Devil's Advocate CoT + 체크리스트 |
| AI Phase 2 (리포트 생성) | Claude Sonnet 4.6 | 3종 리포트 (시민·기자·학생) |
| AI (임베딩) | OpenAI text-embedding-3-small (1536차원) | 텍스트 임베딩 |
...
*§0 현행 운영 현황이 설계 원본보다 우선합니다. 새 세션 시작 시 §0을 먼저 읽으세요.*
*Phase 1-2 상세 구현 코드: `_archive_superseded/DB_CONSTRUCTION_FINAL_PLAN.md`*
*성능 개선 실행계획: `PERFORMANCE_IMPROVEMENT_PLAN_v1.0.md` (작성 예정)*
