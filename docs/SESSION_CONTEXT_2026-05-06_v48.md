...
## TP→TN 오진단 원인 분석 (2026-05-06 확인)

오늘 로컬 파이프라인 테스트에서 명백한 TP 기사 2건이 TN으로 나왔다.
diagnostics 파일 분석 결과 두 가지 독립적 원인이 겹쳐 있음이 확인됐다.

**버그 A — 벡터 검색 누락 (STEP 6에서 해결)**
v3 leaf 107개의 임베딩이 없어서 벡터 검색에 구버전 패턴(1-6-1, 1-1-5 등)만 소환됨.

**버그 B — Sonnet 프롬프트 편향 (STEP 5에서 해결, 최우선)**
`_SONNET_SOLO_PROMPT`의 Few-shot 예시 코드가 전부 구버전(7-5, 3-1 등).
Sonnet이 기사를 감지하더라도 구버전 코드로 출력 → 밸리데이션에서 hallucinated_codes로 탈락 → TN.
diagnostic 증거: `hallucinated_codes: ["7-5", "3-1"]`, `validated_pattern_codes: []`

**핵심 결론**: STEP 5에서 Few-shot 예시 코드를 v3 leaf 형식으로 교체하는 것이
버그 B 해결의 핵심이며, STEP 6보다 먼저 처리해야 한다.
(5인 감리자 전원 STEP 5 선행 동의. 파일: `docs/_scratch/STEP 4 완료 후 진단_*.md`)
