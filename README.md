...
- 📊 **2가지 리포트**: 종합 평가, 기자용 서술형 리포트 생성
- 🚫 **NO SCORING**: 점수나 등급 없이 윤리규범 기반 정성 평가
- ⚡ **하이브리드 AI 전략**: Phase 1(Haiku)에서 빠른 패턴 식별, Phase 2(Sonnet)에서 상세 리포트 생성
- 🎨 **사용자 친화적 UI**: 아코디언 방식의 직관적인 리포트 표시
...
3. "분석하기" 버튼 클릭
4. 2가지 리포트 확인:
   - **평가 결과 종합 리포트**: 기사의 전반적인 윤리 준수 현황
...
├── backend/              # FastAPI 백엔드
│   ├── main.py          # API 엔드포인트
│   ├── analyzer.py      # 기사 분석 로직 (하이브리드 모델 전략)
│   ├── classifier.py    # 기사 유형 분류
│   ├── scraper.py       # 웹 스크래핑
│   ├── export.py        # PDF 변환
