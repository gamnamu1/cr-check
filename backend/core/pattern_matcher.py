...
그들의 집단행동(집회·시위·파업·농성)이 등장하면 다음을 반드시 검토합니다:
- 해당 집단을 겨냥한 명시적 낙인 관용구·비하 어휘('강성 노조', '귀족 노조',
  '생떼', '눈먼 돈' 등)가 제목·기자 서술·인용 어디에든 직접 등장하는 경우
  → 4-3-b
- 명시적 비하어가 없더라도, 해당 집단을 공공질서 파괴자·시민의 적·사회적
  부담·가해자로 위치시키는 프레이밍 → 4-3-b
