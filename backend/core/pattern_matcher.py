...
- 7-2 헤드라인 윤리: 제목이 본문 내용과 다른가? 취재원 발언을 검증 없이 제목으로 뽑았는가?
- 3-1 관점 다양성 부족: 반론·반대 입장이 전혀 없는가?
- 6-1 심층성 부족: 맥락·배경 없이 결론만 있는가?
- 3-4 갈등 조장: 대립 구도를 과장하거나 조장하는가?
...
{{
  ...
  "detections": [
    {{
      ...
      "reasoning": "특정 두 시점의 증가율만으로 국가 간 방역 실태를 비교하는 것은 통계적으로 불충분하다. 실제 절대 수치를 제시하지 않고 증가율만 비교한 것이 핵심 오류다. 이것은 7-3(과장)과 구분해야 한다 — 비교 기준 자체가 통계적으로 무의미한 데이터 오용이다.",
      "severity": "high",
      "pattern_code": "1-5"
    }}
  ]
...
{{
  ...
  "detections": [
    {{
      ...
      "reasoning": "규제 완화에 대한 찬성 전문가만 인용하고, 반대 의견이나 피해 우려를 전혀 다루지 않았다. 1-4(사실과 의견 혼재)와 구분 — 문제는 의견 혼재가 아니라 한쪽 관점만 제시한 것이다.",
      "severity": "high",
      "pattern_code": "3-1"
    }}
  ]
...
{{
  ...
  "detections": [
    {{
      ...
      "reasoning": "'눈먼 돈'은 시각장애인 비하 표현이다. 7-4(자극적 표현)와 구분 — 문제는 선정성이 아니라 특정 장애를 부정적 관용구로 사용하는 차별적 언어이다.",
      "severity": "medium",
      "pattern_code": "7-5"
    }}
  ]
...
{{
  ...
  "detections": [
    {{
      ...
      "reasoning": "'리스크 진행형'은 기자의 해석이지 확인된 사실이 아니다. 6-1(심층성 부족)과의 경계 — 의견을 사실처럼 기술했는가(작위 → 1-4) vs 맥락을 빠뜨렸는가(부작위 → 6-1). 이 기사는 의견을 사실로 포장한 것이므로 1-4가 우선.",
      "severity": "medium",
      "pattern_code": "1-4"
    }}
  ]
...
{{
  ...
  "detections": [
    {{
      ...
      "reasoning": "2,142명 중 폭행 해고자 수가 기사에 없음에도 전체를 폭행범으로 일반화한 제목이다. 3-4(갈등 조장)와 구분 — 이분법적 대결 구도가 아니라 본문에 없는 내용을 제목이 왜곡한 것이 핵심이다.",
      "severity": "high",
      "pattern_code": "7-2"
    }}
  ]
...
{{
  ...
  "detections": [
    {{
      ...
      "reasoning": "AI 생성 콘텐츠의 사실 검증은 편집부의 책임이다. 1-1이 '결과'라면 8-2는 그 '원인'이므로 8-2가 우선 적용된다.",
      "severity": "medium",
      "pattern_code": "8-2"
    }}
  ]
...
```json
{{
  "overall_assessment": "피해자 증언과 문서 증거에 기반한 탐사보도로 공익적 가치가 높다. 개별 패턴 검토: '감금·성폭행' 등 강한 표현은 사건의 심각성에 부합하므로 7-4 해당 아님. 피해자 관점 중심 서술은 인권 탐사보도의 정당한 방법론이므로 3-1 해당 아님. 구체적 위반 근거 없음.",
  "detections": []
}}
```
...
```json
{{
  "overall_assessment": "장기간 현장 취재에 기반한 환경 탐사보도다. 개별 패턴 검토: '지옥이 된 바다'는 구체적 팩트에 근거한 서사적 표현으로 7-3(과장과 맥락 왜곡) 해당 아님. 구체적 위반 근거 없음.",
  "detections": []
}}
```
