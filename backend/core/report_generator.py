...
각 리포트는 다음 분량을 목표로 합니다(한국어 본문 기준).

- comprehensive(시민용/종합): 900~1,300자
...
각 핵심 지적에는 가능한 한 다음 요소를 포함합니다.

1. 기사 본문에서 관찰되는 사안
2. 기사 본문의 근거 문장 또는 표현
3. 관련 규범 근거 (위 「규범 인용 전략」을 따름)
4. 개선 방향

단순 나열이 아니라 관찰 → 기사 근거 → 규범 근거 → 대안이 한 단계씩 쌓이도록 작성합니다.
기사에서 확인되는 사안이 1~2개뿐이면 억지로 3개를 만들지 않습니다. 분량을 채우기 위한 군더더기는 금지합니다.
이 분량·구조 명세는 위 「서술 스타일 가이드」의 중간제목(###) 최대 3~4개 제한과 충돌하지 않습니다.
